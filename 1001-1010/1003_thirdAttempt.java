...
public class Emergency {
	...
	public static void main(String[] args) {
		...
		findRoads(start,end,graph,length,teamNum,visitedCity,rescueTeam,0);
		
		System.out.println(shortRoadNum+" "+maxTeamNum);
		
	}

	private static void findRoads(int currentCity, int endCity, List<List<Pair<Integer, Integer>>> graph,int currentLength,int currentTeamNum,boolean[] visitedCity,int[] rescueTeam,int roadNum) {
		...
		for (Pair<Integer,Integer> pair :graph.get(currentCity)) {
			...
			if(!visitedCity[nextCity]) {
		        ...
		        int newRoadNum = roadNum+1;
				findRoads(nextCity,endCity,graph,newLength,newTeamNum,visitedCity,rescueTeam,newRoadNum);
			}
